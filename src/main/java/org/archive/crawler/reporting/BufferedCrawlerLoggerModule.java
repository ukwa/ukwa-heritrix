...
import org.archive.io.BufferedGenerationFileHandler;
...
/**
 * 
 * Just the same as the CrawlerLoggerModule, but buffers logging events rather
 * that flushing every one.
 * 
 * @author Andrew Jackson <Andrew.Jackson@bl.uk>
 *
 */
public class BufferedCrawlerLoggerModule extends CrawlerLoggerModule {
    @SuppressWarnings("unused")
    private static final long serialVersionUID = 1L;

    protected ConfigPath path = new ConfigPath(Engine.LOGS_DIR_NAME,
            "${launchId}/logs");
    ...
    protected ConfigPath crawlLogPath = new ConfigPath(Logs.CRAWL.getFilename(),
            Logs.CRAWL.getFilename());
    ...
    protected ConfigPath alertsLogPath = new ConfigPath(
            Logs.ALERTS.getFilename(), Logs.ALERTS.getFilename());
    ...
    protected ConfigPath progressLogPath = new ConfigPath(
    ...
    protected ConfigPath uriErrorsLogPath = new ConfigPath(
            Logs.URI_ERRORS.getFilename(), Logs.URI_ERRORS.getFilename());
    ...
    protected ConfigPath runtimeErrorsLogPath = new ConfigPath(
    ...
    protected ConfigPath nonfatalErrorsLogPath = new ConfigPath(
     ...
     *
     * Would contain errors trying to set up a job or failures inside processors
     * that they are not prepared to recover from.
     */
    private transient Logger runtimeErrors;

    /**
     * This logger is for job-scoped logging, specifically recoverable errors
     * which happen and are handled within a particular processor.
     *
    ...
    /**
     * Record of fileHandlers established for loggers, assisting file rotation.
     */
    ...
    public BufferedCrawlerLoggerModule() {
    ...
    protected void setupLogs() throws IOException {
        String logsPath = getPath().getFile().getAbsolutePath()
                + File.separatorChar;
        uriProcessing = Logger.getLogger(LOGNAME_CRAWL + "." + logsPath);
        runtimeErrors = Logger
                .getLogger(LOGNAME_RUNTIME_ERRORS + "." + logsPath);
        nonfatalErrors = Logger
                .getLogger(LOGNAME_NONFATAL_ERRORS + "." + logsPath);
        uriErrors = Logger.getLogger(LOGNAME_URI_ERRORS + "." + logsPath);
        progressStats = Logger
                .getLogger(LOGNAME_PROGRESS_STATISTICS + "." + logsPath);
    ...
    private void setupLogFile(Logger logger, String filename, Formatter f,
        ...
        // Override to use our less flushy logger
        GenerationFileHandler fh = BufferedGenerationFileHandler
                .makeNew(filename,
                false, shouldManifest);
        fh.setFormatter(f);
    ...
    public Logger setupSimpleLog(String logName) {
        Formatter f = new Formatter() {
            public String format(java.util.logging.LogRecord record) {
                return ArchiveUtils.getLog17Date(record.getMillis()) + " "
                        + record.getMessage() + '\n';
            }
        };
     ...
     * 
     * TODO: Its possible for a file to be added twice if reports are force
     * generated midcrawl. Fix.
     *
     * @param file
     *            The filename (with absolute path) of the file to add
     * @param type
     *            The type of the file
     * @param bundle
     *            Should the file be included in a typical bundling of crawler
     *            files.
     *
     ...
     *
     * @param e
     *            URIException encountered
     * @param u
     *            CrawlURI where problem occurred
     * @param l
     *            String which could not be interpreted as URI without exception
     */
    ...
    public void afterPropertiesSet() throws Exception {
        ConfigPath[] paths = { crawlLogPath, alertsLogPath, progressLogPath,
