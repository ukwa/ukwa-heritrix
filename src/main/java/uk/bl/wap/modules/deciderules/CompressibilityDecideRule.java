...
public class CompressibilityDecideRule extends PredicatedDecideRule {
    ...
    /*
     * Create ThreadLocal Deflater instances to avoid contention.
     */
    private static final ThreadLocal<Deflater> threadLocalDeflator = new ThreadLocal<Deflater>() {
        @Override
    ...
    /* Instance of Deflator */
    private final Deflater compresser = threadLocalDeflator.get();
    ...
    protected boolean evaluate(CrawlURI curi) {
        try {
            ...
            double compressibility = ((double) compressedDataLength)
                    / ((double) input.length);
            boolean result = ((compressibility < this.getMin())
                    || (compressibility > this.getMax()));
            return result;
